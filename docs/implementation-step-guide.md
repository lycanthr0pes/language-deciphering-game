...
- 専用のリトライボタンは表示せず、`左クリックでリトライ`の案内だけを表示する。
- 左クリックで全stateを初期化する。
...
- すべての有効なボタンは`onClick`から`playButtonPressSound()`を1回呼び、会話送りと同じ音を鳴らす。無効ボタン、`Space`、背景クリックでは呼ばない。
...
- 暗号単語、候補、解答ボタンをマウスまたは`Enter`で押すと会話送り音が各1回鳴り、無効ボタンでは鳴らない。
