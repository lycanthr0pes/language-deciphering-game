...
- 手帳1見開き最大6件、NEW半周期900ms、判定1400ms、開始2300ms、素材timeout 5000msを置く。
...
- 正答時に問題と送信済み解答を通常例文と同じ形式で重複なく追加し、NEWと書き留め音は発生しない。
...
- `ChoiceList`は男の問題文、暗号トークンと直下の解答枠、日本語候補、選択済み日本語、判定数、送信ボタン、操作案内を1つの統合ダイアログとして描画する。
...
- 未選択、選択済み、選択中、正答、誤答、無効状態が枠とラベルで分かる。
...
- 判定中も統合ダイアログ、残り時間、間違い可能回数を表示する。
- 1400msは表示値を維持したままタイマーと全解答操作を止める。
...
- 現行版は背景、人物、机、手帳、ペンをCSS仮表示し、画像なしで`OpeningBlink`を表示する。
- 将来画像パスを設定した場合は`assetPath()`で読み込み、読込完了後に`OpeningBlink`を表示する。
- 上下まぶたと暗転をCSSで描く。
