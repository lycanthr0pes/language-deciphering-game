...
このゲームでは、`Space`や`Tab`で画面スクロールやフォーカス移動が起きないように使う。
...
| 手帳でTabがブラウザ操作になる | `preventDefault`がない | `event.preventDefault()`を書く |
...
| `cipherText`のようなcamelCaseが多い | `cipher_text`のようなsnake_caseが多い |
...
```python
cipher_words = [token["cipher"] for token in tokens]
```
...
```tsx
<button className={styles.activeToken}>rami</button>
```
...
```python
html = '<button class="activeToken">rami</button>'
```
...
const sampleQuestion = {
  ...
  cipherText: "rami humi",
  tokens: [
    { id: "token-1", cipher: "rami", correctJa: "青い" },
    { id: "token-2", cipher: "humi", correctJa: "女" },
  ],
  correctAnswers: {
    "token-1": "青い",
    "token-2": "女",
...
sample_question = {
    ...
    "cipher_text": "rami humi",
    "tokens": [
        {"id": "token-1", "cipher": "rami", "correct_ja": "青い"},
        {"id": "token-2", "cipher": "humi", "correct_ja": "女"},
    ],
    "correct_answers": {
        "token-1": "青い",
        "token-2": "女",
...
sample_question = {
    "tokens": [
        {"id": "token-1", "cipher": "rami"},
        {"id": "token-2", "cipher": "humi"},
    ],
    "correct_answers": {
        "token-1": "青い",
        "token-2": "女",
