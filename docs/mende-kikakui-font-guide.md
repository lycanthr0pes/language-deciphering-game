# Noto Sans Mende Kikakui 暗号フォント導入案
...
暗号表示へGoogle Fontsの`Noto Sans Mende Kikakui`を採用する場合の、フォント配置、Unicode文字の割り当て、Next.js実装、Figma利用、確認方法を定義する。
...
- `user-select: none`は完全な解析防止にならないため、暗号秘匿の手段として扱わない。
...
以下は実装確認用の仮割り当てである。UI担当が字形を確認し、見分けにくい場合は別のMende Kikakui文字へ変更する。
...
`U+1E800`や`\u{1E800}`という文字列をFigmaへ入力しても字形には変換されない。必ず実際のMende Kikakui文字をコピーして貼り付ける。
...
| --- | --- | --- | --- |
| `color-1` | 赤い | `U+1E800 U+1E806` | `𞠀𞠆` |
| `color-2` | 青い | `U+1E800 U+1E807` | `𞠀𞠇` |
| `quality-1` | 大きな | `U+1E801 U+1E806` | `𞠁𞠆` |
| `quality-2` | 小さな | `U+1E801 U+1E807` | `𞠁𞠇` |
| `quantity-1` | いくつかの | `U+1E802 U+1E806` | `𞠂𞠆` |
| `quantity-2` | たくさんの | `U+1E802 U+1E807` | `𞠂𞠇` |
| `verb-1` | 見る | `U+1E803 U+1E806` | `𞠃𞠆` |
| `verb-2` | 追う | `U+1E803 U+1E807` | `𞠃𞠇` |
| `humanNoun-1` | 男 | `U+1E804 U+1E806` | `𞠄𞠆` |
| `humanNoun-2` | 女 | `U+1E804 U+1E807` | `𞠄𞠇` |
| `animalNoun-1` | 犬 | `U+1E805 U+1E806` | `𞠅𞠆` |
| `animalNoun-2` | 猫 | `U+1E805 U+1E807` | `𞠅𞠇` |
...
| ID | 決定項目 | 推奨案 | PM決定 |
| --- | --- | --- | --- |
| M01 | フォント採用 | `Noto Sans Mende Kikakui`を採用 |  |
| M02 | 配信方法 | ローカル配置と`next/font/local` |  |
| M03 | 使用文字数 | カテゴリ6文字、候補2文字の合計8文字 |  |
| M04 | 文字構造 | `カテゴリ文字 + 候補文字` |  |
| M05 | 単語内部の方向 | 右から左 |  |
| M06 | 問題内の単語順 | 左から右 |  |
| M07 | 仮コードポイント | `U+1E800`から`U+1E807` |  |
| M08 | ゲーム内表現 | メンデ文字を用いた暗号表記 |  |
