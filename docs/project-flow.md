...
- Lv1からLv12まで進み、各レベルで例文提示、問題提示、解答を行う。
- EASYは各問題で1回まで誤答後も継続できる。HARDは最初の誤答または時間切れで即ゲームオーバーとする。
...
flowchart TD
    ...
    C -->|難易度選択後にSTART・RunDefinition生成・startedAt保存| D[CSSまばたき演出]
    D --> E[導入会話]
    E --> F[現在Lvの暗号例文を提示]
    F --> G[問題提示]
    G --> H[問題文をクリック]
    H --> I[例文を履歴へ一括追加・NEW・書き留め音・内外を分けた解答UIとステータスを表示]
    I -->|HARDの時間切れ| N[ゲームオーバー発砲演出]
    I --> J[解答する]
    J --> K{判定結果}
    K -->|EASYの誤答・継続可| R[正答数と色分けを表示して320ms揺らし即時再開]
    R --> I
    K -->|正答・Lv1〜11| L[正答数と色分けを1400ms表示]
    L --> T[次Lvへ]
    T --> F
    K -->|正答・Lv12| S[正答数と色分けを1400ms表示]
    S --> M[クリア発砲演出]
    K -->|誤答・終了条件| N
    M --> O[GAME CLEAR]
    N --> P[GAME OVER]
...
```mermaid
flowchart TD
    A[初回起動・再読込・再マウント] --> B[フォント・Figma照明・統一済み人物と机を読み込む]
    B --> C{クリティカル素材が5000ms以内に読めたか}
    C -->|いいえ| D[エラーと再読込案内]
    C -->|はい| E[メインメニュー]
    E --> F[難易度を選択してSTART]
    F --> G[RunDefinitionを生成してstartedAtを保存]
    G --> H{モーション低減か}
    H -->|いいえ| I[2300msのCSSまばたき]
    H -->|はい| J[300msフェード]
    I --> K[完了通知]
    J --> K
    K --> L[導入会話開始]
...
初回起動、ブラウザ再読込、開発時の再マウントではメインメニューに留まり、自動で開始演出や導入会話へ進まない。メニュー表示中は開始演出を再生せず、リザルトから戻った後を含めて`START`を押すたびに再生する。演出中は会話、手帳、解答を受け付けない。完了は専用`animationend`と+250msのフォールバックを同じ一度きりのガードへ接続する。Figmaは照明と配置の正本、人物・机は`public/assets/images`の現行PNGを画風の正本とし、人物差分切替時に実行時フィルターを加えない。
...
タイマーは`answering`中だけ減る。0になった同じタイマー処理で操作を停止し、判定表示を挟まず失敗演出へ進む。次レベルでは120秒へ初期化する。
...
- メニューから開始演出、導入、Lv1〜Lv12、クリアまで進める。
- 初回起動・同一タブのブラウザ再読込・開発時の再マウントでメニューに留まり、リザルトから戻った場合を含めて`START`を押すたびに開始演出が再生されることを確認する。
