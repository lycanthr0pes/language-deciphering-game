...
この文書は、現行テスト版を完成版へ拡張するための承認済み仕様と実装分担を定義する。完成版で追加・変更する内容は本書を正とし、本書に記載しない既存仕様は`game-rule.md`、`mende-kikakui-font-guide.md`、`ui-spec.md`、`implementation-spec.md`、`project-flow.md`を正とする。

本書の内容はコード実装へ採用できる。既存のメイン資料へ統合する際は、完成版の追加・変更内容を本書に合わせ、既存仕様との境界を崩さない。

タスク管理は`final-version-task-sheet.md`、非規範の実装例は`final-version-sample-code.md`を参照する。

## 2. 完成版の変更概要

- Figmaのレイアウトを正として参考にする。
- 起動時にメインメニューを追加する。
...
- 背景は`#020202`の黒地と、Figma 由来の、タイトルと被らないように明るさを調整した`dark-scene-ceiling-light.svg`だけで構成する。
...
メインメニューは画面遷移を増やさず、難易度選択画面は同じ画面内のパネル(Figmaで作成した半透明のモーダル)に切り替える。
...
- `正答 n / N`を表示する。
- `正答 n / N`は問題・解答パネルより上へ、枠や問題文と重ならない間隔を空けて表示する。
- 正答欄を緑、誤答欄を赤で表示する。
